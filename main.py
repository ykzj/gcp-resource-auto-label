...
COMPUTE_INSTANCE_NAME_LABEL_KEY="resourcename"
COMPUTE_INSTANCE_ID_LABEL_KEY="resourceid"
CONTAINER_CLUSTER_LABEL_KEY="cluster"
STORAGE_BUCKET_LABEL_KEY="bucket"
SQLADMIN_INSTANCE_LABEL_KEY="resourceid"

# Label Compute Engine VMs
# https://cloud.google.com/compute/docs/instances/instance-life-cycle
def label_compute_instance(asset_name,asset_resource_data_status,asset_resource):

    # Process only when it is "PROVISIONING"
    if asset_resource_data_status == "PROVISIONING":
        instance_name_key = COMPUTE_INSTANCE_NAME_LABEL_KEY
        instance_id_key = COMPUTE_INSTANCE_ID_LABEL_KEY

        # Here is a sample asset_name
        # "//compute.googleapis.com/projects/lxd-firebase/zones/us-central1-a/instances/auto-label-test"

        # Extract the properties from the asset name
        pattern = re.compile(r".*\/projects\/(?P<project_id>.*?)\/zones\/(?P<zone>.*?)\/instances\/(?P<instance_name>.*?)$", re.VERBOSE)
        ...
        if instance_id_key in labels and labels[instance_id_key] == asset_resource['id']:
            print("The same instance_id label key-value already exists.")
        elif instance_name_key in labels and labels[instance_name_key] == insatnce_name:
            print("The same instance_name label key-value already exists.")
        else:
            # Use the instance_id as the label value
            labels[instance_id_key] = asset_resource['id']
            labels[instance_name_key] = instance_name
            ...
            print("Finished setting labels on {}".format(instance_name))
            print({"service_set_labels_response":service_set_labels_response})

            # print("disks: ",asset_resource["disks"])

            for disk in asset_resource['disks']:
                # print("disk: ",disk)
                disk_id = disk['source'].split('/')[-1]
                # print("disks_id: ",disk_id)
                disk_get_response={}
                try:
                    disk_get_response=service.disks().get(
                        project=project_id,
                        zone=zone,
                        disk=disk_id
                    ).execute()
                    print({"disk_get_response":json.dumps(disk_get_response)})
                except HttpError as exception:
                    if exception.resp["status"] == "404":
                        # exit gracefully if encountering 404
                        # One reason is due to the Compute Engine VMs created by GKE Autopilot clusters, 
                        # which generate Compute Engine Instance notifications but are not available via computer API
                        print("Cannot find disk_id={} . Exiting gracefully.".format(disk_id))
                        return
                    else:
                        raise exception

                labelFingerprint=disk_get_response["labelFingerprint"]
                labels={}

                if "labels" in disk_get_response:
                    labels = disk_get_response["labels"]

                labels[instance_id_key] = asset_resource['id']
                disk_set_labels_response = service.disks().setLabels(
                    project=project_id,
                    zone=zone,
                    resource=disk_id,
                    body={
                ...
                print("Finished setting disk labels on {}".format(disk_id))
                print({"disk_set_labels_response":disk_set_labels_response})
...
def auto_resource_labeler(event, context):
    ...
    try:
        ...
        asset_resource = {}
        if "resource" in message_object["asset"] and "data" in message_object["asset"]["resource"]:
            asset_resource = message_object["asset"]["resource"]["data"]
        ...
        # else (not deleted)
            ...
            if asset_type == "compute.googleapis.com/Instance":
                ...
                label_compute_instance(asset_name,asset_resource_data_status,asset_resource)
